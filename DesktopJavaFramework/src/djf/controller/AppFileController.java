...
import static djf.settings.AppStartupConstants.OLD_PATH;
import static djf.settings.AppStartupConstants.PATH_WORK;
import static djf.settings.AppStartupConstants.REP_PATH;
import static djf.settings.AppStartupConstants.SOURCE_PATH;
...
if (saveas != null) {
    ...
    public void handleExportRequest() throws IOException {
       ...
       try {
            // WE MAY HAVE TO SAVE CURRENT WORK
            boolean continueToExoport = true;
            if (!saved) {
                // THE USER CAN OPT OUT HERE
                continueToExoport = promptToSave();
            }

            // IF THE USER REALLY WANTS TO EXIT THE APP
            if (continueToExoport) {
                File old=null;
                if(currentWorkFile!=null)
                   old = currentWorkFile;
                else{
                   if(new File(OLD_PATH)!=null)
                        old = new File(OLD_PATH);
                   else
                       dialog.show("Loading error", "Loading error");
                } 
                    
       File rep = new File(REP_PATH);
       Files.copy(old.toPath(), rep.toPath(), StandardCopyOption.REPLACE_EXISTING);
       ...
       if(exportDir==null)
           ...
           dc.setInitialDirectory(exportDir);
       ...
       if(selectedDir != null){
         ...
         new SimpleFileVisitor<Path>() {
             public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs)
                 throws IOException
             {
                 Path targetdir = selectedDir.toPath().resolve(source.toPath().relativize(dir));
                 try {
             ...
             public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                 throws IOException
             {
                 Files.copy(file, selectedDir.toPath().resolve(source.toPath().relativize(file)),StandardCopyOption.REPLACE_EXISTING);
                 return CONTINUE;
             }
         });
       }
	    ...
	    //AppMessageDialogSingleton dialog = AppMessageDialogSingleton.getSingleton();
	    //PropertiesManager props = PropertiesManager.getPropertiesManager();
	    dialog.show("export error", "export error");
        }
	...
	fc.setTitle(props.getProperty(LOAD_WORK_TITLE));
		...
		app.getWorkspaceComponent().activateWorkspace(app.getGUI().getAppPane());
                ...
                //if load successful, curretnWrokfile = selectfile
                currentWorkFile = selectedFile;
